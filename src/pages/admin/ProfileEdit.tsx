...
import AxiosInstance from '../../lib/api'; // <-- adjust path to where AxiosInstance is
...
export default function EditProfile() {
  ...
  // Fetch profile from backend using AxiosInstance
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // Real API call using AxiosInstance
        const res = await AxiosInstance.get<ProfileData>('/api/profile');
        setProfile(res.data);
      } catch (err) {
        console.error('Failed to load profile', err);

        // Fallback to localStorage mock if backend is not ready
        const mockData = localStorage.getItem('userProfile');
        if (mockData) {
          setProfile(JSON.parse(mockData));
        }
      }
    };
  ...
  const updateProfile = async () => {
    ...
    try {
      // Real API call using AxiosInstance
      await AxiosInstance.put('/api/profile/update', profile);

      // Save locally only for mock/demo purposes
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-6 flex justify-center">
      <div className="w-full max-w-4xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-8">
          {['firstName', 'lastName', 'email', 'phone', 'country', 'city', 'postalCode', 'taxId'].map(
            (field) => (
              <input
                key={field}
                name={field}
        ...
        <div className="flex justify-center">
          <button
            ...
            disabled={loading}
            className=" text-white px-10 py-3 rounded-full font-medium hover:bg-green-800  "
          >
            {loading ? 'Saving...' : 'Save'}
          </button>
        </div>
